...
**SKIP은 자신의 업무 문제를 이해하는 사람이 코딩 에이전트와 소프트웨어를 만드는 워크플로입니다.** 사람은 원하는 결과와 제품 규칙을 결정하고, 에이전트는 저장소 조사·설계·구현·검증을 맡습니다. 결정과 근거를 기록해 다음 대화에서도 작업을 이어갑니다.

도메인 전문가, 운영자, 분석가, 코딩 에이전트와 일하는 1인 개발자를 위해 만들었습니다. **무엇을 만들고 싶은지는 알아야 합니다. 어디를 어떻게 고칠지까지 알 필요는 없습니다.**

> **다음 버전 개발본:** SQLite 공통 Core·MCP·현재 대화 실행 UI를 구현하고 있습니다. [구조와 검증·지원 범위](docs/ko/db-core.md)를 확인하세요. 기존 설치본과 실제 Windows 앱 검증은 별개입니다.
