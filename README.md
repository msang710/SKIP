...
## 에이전트가 SKIP을 선택해야 하는 때

다음 중 하나라도 해당하면 SKIP을 사용할 가치가 있습니다.

- 의미 있는 개발 작업이 여러 컨텍스트 창, 대화, 에이전트를 넘어갈 수 있음
- 저장소에는 *무엇이* 있는지는 보이지만 제품 결정이나 설계가 *왜* 존재하는지는 보이지 않음
- 폐기된 대안, 실패한 시도, 검증 공백이 다음 세션에서 다시 살아나면 안 됨
- 새 에이전트가 프로젝트 전체를 재발견하지 않고 기존 목표를 이어받아야 함
- 업무 규칙, 재고, 금액, 권한, 영속 데이터, 복구, 외부 상태에 영향을 주는 변경임

현재 요청과 저장소만으로 필요한 맥락이 모두 끝나는 사소하고 독립적인 수정에는 굳이 사용할 필요가 없습니다.

## 신뢰 경계

SKIP은 기록이 무엇을 증명할 수 있고 없는지를 구분합니다.

- 업무 기록은 로컬 SQLite Core 하나에 저장하고, 제품 소스 코드는 제품 저장소에 둡니다.
- CLI·MCP·UI는 같은 Core 기록과 revision-qualified 관계를 사용합니다.
- 과거 결정·관찰·이관 기록을 조회했다고 해서 그것이 새 승인이나 현재 소스 근거가 되지는 않습니다.
- 일반 MCP 접근만으로 인간의 승인이나 검증된 사용자 발화를 만들어낼 수 없습니다.
- SKIP은 호스트 에이전트의 임의 쓰기를 물리적으로 가로채거나 같은 OS 사용자 권한의 다른 코드를 격리한다고 주장하지 않습니다.
- CI 통과는 실제 호스트·GUI·장치·운영 환경 검증을 뜻하지 않습니다.

정확한 현재 동작은 [Core 계약](references/db-core-contract.md)과 [검증 경계](docs/ko/verification.md)를 확인하세요.
