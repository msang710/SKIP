...
Work-item `request_id` identifies why the work was recorded; it is not a permanent execution prohibition. A later explicit implementation request authorizes execution of the existing work through a separate authorization request. Keep the planning request and work identity unchanged. Do not duplicate tasks or rewrite their provenance just to escape a planning-only restriction. Current instructions, product decisions, linked requirements, scope and fresh evidence still apply.

Input-ID and request-ID interpretation tools address the same semantic history. Read the returned revision before amending it; do not create a second interpretation copy for execution. A continuation can use a saved implementation request before the first execution starts. Multiple selected works form a permitted set; selected plans or requirements permit only work connected to their exact revisions. Resolve conflicting historical interpretations explicitly instead of searching for an older permissive one.
