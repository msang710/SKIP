...
export function SettingsPanel({theme,projectId,load,save,pinGoals=false,onPinGoals}:{pinGoals?:boolean;onPinGoals?:(value:boolean)=>void;theme:any;projectId:string;load:()=>Promise<any>;save:(payload:any,key:string)=>Promise<any>}) {
  ...
  return <ScrollView style={{flex:1}} contentContainerStyle={{padding:24,gap:16,maxWidth:900,width:"100%",alignSelf:"center"}}>
    ...
    {onPinGoals?<View style={{gap:6,paddingBottom:12}}>
      <Text style={{...text,fontWeight:"600"}}>화면</Text>
      <Pressable accessibilityRole="switch" accessibilityLabel="목표 목록 고정" accessibilityState={{checked:pinGoals}} onPress={()=>onPinGoals(!pinGoals)} style={{paddingVertical:10,flexDirection:"row",gap:10}}>
        <Text style={{color:pinGoals?theme.colors.accent:muted}}>{pinGoals?"☑":"☐"}</Text><Text style={text}>목표 목록 고정</Text>
      </Pressable>
      <Text style={{color:muted}}>모든 프로젝트에 적용됩니다. 넓은 화면에서 목표 목록을 본문 옆에 표시합니다.</Text>
    </View>:null}
