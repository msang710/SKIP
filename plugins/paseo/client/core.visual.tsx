...
export function Action({label,onPress,disabled=false,theme,accessibilityLabel}:{accessibilityLabel?:string;label:string;onPress:()=>void;disabled?:boolean;theme:any}) {
  return <Pressable accessibilityRole="button" accessibilityLabel={accessibilityLabel??label} accessibilityState={{disabled}} disabled={disabled} onPress={onPress} style={({pressed})=>({paddingVertical:10,paddingHorizontal:14,borderRadius:10,borderWidth:1,borderColor:tint(theme.colors.foregroundMuted,"30"),backgroundColor:pressed?tint(theme.colors.accent,"24"):theme.colors.surface0,opacity:disabled?.45:1,transform:[{scale:pressed?.985:1}]})}><Text style={{color:theme.colors.foreground,fontSize:13,fontWeight:"600"}}>{label}</Text></Pressable>;
}
