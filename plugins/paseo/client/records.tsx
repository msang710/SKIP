...
import { getDecisionInbox, getIntentRecordAttachment, listIntentRecords, readIntentRecord, readRecordGroup } from "../shared/records";
import { projectReading, statusLabel, type RecordMeta, type Excerpt } from "../shared/records.presentation";
import { MarkdownDocument } from "./records.document";
import { recordCaller } from "../shared/records.caller";
...
function Reader({ theme, layout, workspaceId }: PluginWorkspacePanelProps) {
  ...
  const attach = async (record: RecordMeta) => {
    ...
    try {
      const caller=recordCaller(record);try{await copyText(caller);setNotice("호출자를 복사했습니다.");}catch{setNotice(caller);}
    ...
    <View style={{ flex: 1, minHeight: 0, flexDirection: narrow ? "column" : "row", gap: 28 }}>
      ...
      {(chosen || doc) ? <View style={{ flex: 1, minWidth: 0, gap: 12 }}>
        ...
        {doc ? <>
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>{button(attaching ? "첨부 중…" : "호출자 복사", () => void attach(doc.record), true)}</View>
