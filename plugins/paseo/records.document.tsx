...
export function InlineMarkdown({ text, theme }: { text: string; theme: PluginTheme }) {
  ...
  return <Text>{tokens.map((token, index) => {
    ...
    if (token.startsWith("*") && token.endsWith("*")) return <Text key={index} style={{ fontStyle: "italic" }}>{token.slice(1, -1)}</Text>;
    if (token.startsWith("~~") && token.endsWith("~~")) return <Text key={index} style={{ textDecorationLine: "line-through" }}>{token.slice(2, -2)}</Text>;
...
export function MarkdownDocument({ markdown, theme, compact, embedded = false }: { markdown: string; theme: PluginTheme; compact: boolean; embedded?: boolean }) {
  const scroll = useRef<ScrollView>(null);
  const [showContents, setShowContents] = useState(false);
  const positions = useRef<Record<number, number>>({});
  const lines = (embedded ? markdown : markdown.replace(/^---\n[\s\S]*?\n---\n/, "")).replaceAll("\r\n", "\n").split("\n");
  ...
  for (const line of lines) {
    ...
    else if (bullet) { flush(); blocks.push({ type: "bullet", text: bullet[1].replace(/^\[ \]\s*/, "☐ ").replace(/^\[[xX]\]\s*/, "☑ ") }); }
  ...
  return embedded ? content : <ScrollView ref={scroll} style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 36 }}>{content}</ScrollView>;
}

/** Format one stored text field without constructing or retrieving a document. */
export function MarkdownField({ text, theme }: { text: string; theme: PluginTheme }) {
  return <MarkdownDocument markdown={text} theme={theme} compact embedded />;
}
