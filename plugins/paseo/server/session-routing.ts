...
import { CoreClient, coreRoot, type Bridge } from "./core.bridge";
...
export class CoreSessions {
  ...
  async connect(api: PaseoApi, binding: Binding) {
    ...
    try {
      await bridge.call({ operation: "connect", project_id: observed.projectId, sources: { main: observed.directory }, identity: observed.identity, participant: observed.identity[4] && observed.identity[5] ? {namespace:"paseo-"+createHash("sha256").update(JSON.stringify([os.hostname(),os.homedir(),observed.identity[3]])).digest("hex"),agent_id:observed.identity[4],session_id:observed.identity[5]} : null, can_start: observed.canStart });
