...
Explicit actions must be bound to the selected goal or exact work. A `resume` action must name the exact work and inherit a permitted action from that work's original verified request in the same Codex conversation. Both current and original constraints remain applicable; a planning request cannot become implementation through `resume`. The native adapter rechecks the semantic basis when Core enters its transaction. Changed input, selected record revision or interpretation requires refresh. Finish reports the existing execution in the same conversation and never starts another action.
